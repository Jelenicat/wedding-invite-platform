...
function SplitVideoIntro({
  ...
  const themeStyles = {
    ...
    "--intro-button-text":
      theme.introButtonText || theme.buttonText || "#000000",
    "--intro-button-border":
      theme.introButtonBorder || "rgba(20, 20, 20, 0.28)",
    ...
    "--intro-text-shadow":
      theme.introTextShadow || "rgba(0, 0, 0, 0.18)",
    "--intro-overlay":
      theme.introOverlay || "rgba(255, 252, 248, 0.05)",
    "--intro-play-bg":
      theme.introPlayBg || "rgba(255, 255, 255, 0.2)",
  ...
  return (
    <section
      className={`split-video-intro ${
        isCyrillic ? "split-video-intro--cyrillic" : ""
      }`}
      style={themeStyles}
    >
      <video
      ...
      <div className="split-names">
        <span>{brideName}</span>
        <span className="split-and">{t.and}</span>
        <span>{groomName}</span>
      </div>
