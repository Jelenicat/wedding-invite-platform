...
function ItalianRSVP({
  ...
  const labels = {
    ...
    subtitle: isCyrillic
      ? "Молимо вас да свој долазак потврдите на време."
  ...
  const handleSubmit = async (e) => {
    ...
    if (!slug || !eventType) {
      console.error("Nedostaje slug ili eventType za RSVP:", {
        slug,
        eventType,
      });
      setStatus("error");
    ...
    try {
      ...
      console.error("Greška pri slanju Italian RSVP:", error);
      setStatus("error");
    }
  };
      ...
      >
        <AnimatePresence mode="wait">
          {submitted ? (
            <motion.div
              key="success"
              className="italian-rsvp-success"
            ...
            >
              <motion.div
                className="italian-rsvp-success-heart"
              ...
              <div className="italian-rsvp-confetti-wrap">
                {Array.from({ length: 18 }).map((_, i) => (
                  <motion.span
                    key={i}
                    className="italian-rsvp-confetti"
                    initial={{
                      opacity: 0,
                      y: 0,
                      x: 0,
            ...
            <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <div className="italian-rsvp-heading">
                <p className="italian-rsvp-eyebrow">{labels.eyebrow}</p>

                <h2 className="italian-rsvp-title">{labels.title}</h2>

                <p className="italian-rsvp-subtitle">{rsvpSubtitle}</p>

                <div className="italian-rsvp-ornament">
                  <span />
                  <em>♡</em>
                  <span />
                </div>
              ...
              <form className="italian-rsvp-form" onSubmit={handleSubmit}>
                <label className="italian-rsvp-field">
                ...
                <div className="italian-rsvp-attendance">
                  <button
                    type="button"
                    className={`italian-rsvp-attendance-card ${
                      form.attending === "yes" ? "is-active" : ""
                    }`}
                    onClick={() => handleChange("attending", "yes")}
                    disabled={status === "sending"}
                  >
                    <span className="italian-rsvp-choice-icon">♡</span>
                ...
                <AnimatePresence initial={false}>
                  {form.attending === "yes" && (
                    <motion.div
                      className="italian-rsvp-guests-box"
                    ...
                    >
                      <div>
                        <span className="italian-rsvp-guests-label">
                          {labels.guests}
                        </span>
                        <p>{labels.guestsNote}</p>
                      </div>

                      <div className="italian-rsvp-stepper">
                        <button
                          type="button"
                ...
                {status === "error" && (
                  <p className="italian-rsvp-status is-error">
                    {labels.error}
                  </p>
                )}
              </form>
