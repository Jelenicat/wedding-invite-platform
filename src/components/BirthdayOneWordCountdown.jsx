...
function BirthdayOneWordCountdown({ targetDate, backgroundImage }) {
  ...
  return (
    <section
      ...
      style={
        backgroundImage
          ? { "--birthday-one-word-countdown-bg": `url(${backgroundImage})` }
          : undefined
      }
    >
      <div className="birthday-one-word-countdown-bg-image" />
      <div className="birthday-one-word-countdown-overlay" />

      <motion.div
        className="birthday-one-word-countdown-card"
        initial={{ opacity: 0, y: 26 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, amount: 0.2 }}
        transition={{ duration: 0.8 }}
      >
        <p className="birthday-one-word-countdown-kicker">COUNTDOWN</p>

        <h2 className="birthday-one-word-countdown-title">
        ...
        {timeLeft ? (
            ...
            {units.map((item) => (
              <div className="birthday-one-word-countdown-unit" key={item.label}>
                <AnimatePresence mode="popLayout">
                  <motion.span
                    key={item.value}
                    className="birthday-one-word-countdown-number"
          ...
          <motion.div
            className="birthday-one-word-countdown-finished"
...
export default BirthdayOneWordCountdown;