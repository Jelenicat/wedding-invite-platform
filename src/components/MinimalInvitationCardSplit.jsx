...
function MinimalInvitationCardSplit({
  brideName,
  groomName,
  details = {},
  backgroundImage,
  ...
  const hasGeneralPalette = details.dressCodePalette?.length > 0;
  const hasWomenPalette = details.dressCodeWomenPalette?.length > 0;
  const hasMenPalette = details.dressCodeMenPalette?.length > 0;
        ...
        <div className="minimal-invitation-paper">
          ...
          {shouldShowDressCode && (
            <div className="minimal-extra-card minimal-dresscode-editorial">
                  ...
                  {hasWomenPalette && (
                    <div className="minimal-palette-box">
                      <div className="minimal-palette minimal-palette-editorial">
                        {details.dressCodeWomenPalette.map((color, index) => (
                          <span
                            key={`women-${color}-${index}`}
                            className="minimal-palette-dot minimal-palette-dot-editorial"
                            style={{ backgroundColor: color }}
                            aria-label={`women dress code color ${index + 1}`}
                          />
                        ))}
                      </div>
                    </div>
              ...
              {hasGeneralPalette && !hasWomenPalette && !hasMenPalette && (
                <div className="minimal-palette-box">
                  <div className="minimal-palette minimal-palette-editorial">
                    {details.dressCodePalette.map((color, index) => (
                      <span
                        key={`general-${color}-${index}`}
                        className="minimal-palette-dot minimal-palette-dot-editorial"
                        style={{ backgroundColor: color }}
              ...
              {details.dressCodeMen && (
                <div className="minimal-dresscode-role minimal-dresscode-role-men">
                  ...
                  {hasMenPalette && (
                    <div className="minimal-palette-box">
                      <div className="minimal-palette minimal-palette-editorial">
                        {details.dressCodeMenPalette.map((color, index) => (
                          <span
                            key={`men-${color}-${index}`}
                            className="minimal-palette-dot minimal-palette-dot-editorial"
                            style={{ backgroundColor: color }}
                            aria-label={`men dress code color ${index + 1}`}
                          />
                        ))}
                      </div>
                    </div>
...
export default MinimalInvitationCardSplit;