...
function MinimalInvitationCard({ brideName, groomName, details = {} }) {
      ...
      <div className="minimal-invitation-paper">
        ...
        {infoItems.length > 0 && (
          <div className="minimal-program-card">
            <h3 className="minimal-section-title">Plan vencanja</h3>

            <div className="minimal-program-list">
              {infoItems.map((item, index) => (
                <div className="minimal-program-row" key={`${item.label}-${index}`}>
                  <span className="minimal-program-row-label">{item.label}</span>
                  <span className="minimal-program-row-value">{item.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {details.dressCodePalette?.length > 0 && (
          <div className="minimal-extra-card">
            ...
            {details.dressCodeNote && (
              <p className="minimal-section-note">{details.dressCodeNote}</p>
            )}
          </div>
        )}
